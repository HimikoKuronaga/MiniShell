...
int returning;         /* 1 si se ve la proposiol�n de retorno */

typedef struct Frame
{               /* nivel da pila par� Huida a proc/func */
   Symbol *sp;  /* entrada en la tabla da simbolos */
   Inst *retpc; /* d�nde reanudar despu�s del retorno */
   Datum *argn; /* n-esimo argumento en la pila */
   int nargs;   /* n�mero da argumentos */
} Frame;
...
{
   sp->u.defn = (Inst)progbase; /* lnlolo de c�digo */
...
void funcret()
{
   ...
   d = pop(); /* conservar el valor de retorno de la funol�n */
   ret();
   push(d);
}
